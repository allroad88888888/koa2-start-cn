...
export default class AlloyFinger extends React.Component {
    ...
    _emitEvent(name, e,target){
        if (this.props[name]) {
            this.props[name](e,target);
        }
    }
    ...
    _handleTouchEnd(evt){
       ...
       // console.log("啊哈  娘子")
        ...
        if( evt.touches.length<2){
            this._emitEvent('onMultipointEnd', evt,target);
        }
        ...
        if ((this.x2 && Math.abs(this.x1 - this.x2) > 30) ||
            ...
            this.swipeTimeout = setTimeout(function () {
                self._emitEvent('onSwipe', evt,target);
            }, 0)
        } else {
            this.tapTimeout = setTimeout(function () {
                self._emitEvent('onTap', evt,target);
                if (self.isDoubleTap) {
                    self._emitEvent('onDoubleTap', evt,target);
